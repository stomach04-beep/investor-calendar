...
| 順 | 日本株 | 米国株 | 確度 |
|---|---|---|---|
| 1 | JPX 公式発表予定（約1ヶ月先まで） | Nasdaq カレンダー（約5週先まで）で**時間帯あり**の行 | **確定**（会社が登録・確認した日程。`is_estimated=false`） |
| 1' | — | Nasdaq カレンダーで時間帯なしの行（`Nasdaq見込み`） | 見込み。2,889行中2,058行が「昨年+364日」ちょうど＝機械予測（2026-09-23 実測） |
| 2 | yfinance | yfinance | Yahoo の予定日（推定を含む） |
| 3 | J-Quants 開示履歴からの予測 | SEC EDGAR 8-K(2.02) 実績からの予測 | 下記の実測精度 |

### 2026-09-23 の仕様見直し（日付と時間帯の正確さ）

- **「Nasdaq公式」は大半が機械予測だった**。時間帯（寄り前/引け後）の無い行は AAPL/MA/ICE/HSY/KMB
  など主力株でも「昨年の発表日+364日」で、会社確認済みではない。時間帯あり＝確定、なし＝見込みに分けた。
- **曜日のクセで未確定の日付を寄せる**。直近8回すべて同じ曜日の銘柄は次回もその曜日に出す率が
  米国株 92%（EDGAR実績 43銘柄）・日本株 82%（J-Quants 10年開示履歴）。7/8 だと 80%/68%、6/8 だと 63%/55% に
  落ちるので「8回全部」だけ使う。確定以外の日付がその曜日でなく ±3日以内にその曜日の営業日があれば寄せ、
  説明欄に注記する（例: NVDA は水曜8/8なのに Yahoo が 11/17(火) を返した → 11/18(水)）。
  日本株の直近8回は `jq_earnings_jp.json` の `recent_disc`（`build_earnings_estimates.py` が出力）。
- **米国株の時刻は「早い側の上限」**。8-K 受理時刻は発表より必ず遅い（提出は発表の後。中央値だと
  JNJ 6:45発表→7:46、ITW 8:00→9:14、VLO 6:30→8:09、BR 7:00→7:59 と 1〜1.5時間遅れる銘柄がある）。
  Yahoo の過去実績の時刻は「時の位で切り捨て」（6:45→06:00）で発表より遅くならない。
  両方あるときは早い方（同セッションの 8-K 最小値 vs Yahoo）を採る。通知が発表の後に鳴るのを避けるため、
  早い側に外す。実測5銘柄で遅れゼロ・最大45分早いだけ。説明欄に「Yahoo 06:00／8-K受理 06:35〜07:46」と幅を書く。
- **件名と説明欄に時間帯と日本時間**。件名は「○○ 決算（引け後）」、説明欄は
  「米国 10/29(木) 引け後 16:00 ET ＝ 日本時間 10/30(金) 05:00」。日本株は 東証 15:30 引け基準で
  寄り前/場中/昼休み/引け後（従来の既定 15:00 は「場中」）。
- **Notion に「日付履歴」を残す**（`notion_upsert.py`）。採用日か出どころが変わった日に
  「2026-09-23:2026-10-29|Nasdaq見込み」を追記する。決算通過後に実際の発表日と突き合わせて
  出どころ別の的中率を数えるための材料（`scripts/earnings_ledger_score.py`）。公開JSONには出ない。
...
候補が複数あるときの絞り方が2通りあり、いまは日米とも
「前回発表 + 91日×n」で見積もった次の四半期に**最も近い**候補を採る（`pick_next_date`）。

`python scripts/earnings_accuracy.py` で測り直せる（過去の実発表日ごとに、
その時点より前の履歴だけで予測を作って突き合わせるアウトオブサンプル検証）。

**(a) ルール自体の比較**（10年ぶんの実発表日を全部候補にした場合）
...
候補が生の履歴のままなら差は大きい。決算以外の開示が1件混ざるだけで
「最も早い候補」は3ヶ月手前の日付を拾うため（平均ズレ 8.9日 対 2.5日）。
米国株の EDGAR 予測はまさにこの生履歴を候補にするので、この差がそのまま効く。

**(b) 日本株が実際に通る経路**（`data/jq_earnings_jp.json` の候補日リストを使う場合）

| 検証 | 件数 | ピタリ | ±1日 | ±3日 | ±7日 | 平均ズレ |
|---|---|---|---|---|---|---|
| 日本株・最も早い候補 | 135,715 | 33.3% | 63.3% | 80.9% | 96.3% | 2.68日 |
| 日本株・四半期に合わせる | 135,715 | 33.3% | 63.3% | 80.8% | 96.2% | 2.76日 |

⚠️ **日本株ではルールを変えても数字は動かない**（2026-08-27 実測。差は 135,715 件中 18 件＝0.01%）。
`jq_earnings_jp.json` は `jquants-bulk/build_earnings_estimates.py` が
**次の4四半期を1四半期1本ずつ**に絞って書き出しているため、
「今日以降で最も早い候補」の時点ですでに四半期に合っているから
（実ファイルの候補間隔は中央値91日。60日未満の間隔は 13,446 本中 36 本だけ）。
つまり日本株は以前から (a) の下段と同じ 2.7日 相当で動いており、
上表 (a) 上段の 8.9日 は日本株の実力ではない。

それでも日米で同じ `pick_next_date` を通しているのは、
1) 候補の選び方が2か所にあるとズレるから（同じ判定を2回書かない）、
2) 生成側が1四半期1本を崩したとき（実ファイルにも 36 本ある）に受け側で吸収できるから。
そのために `jq_earnings_jp.json` は起点 `last_disc`（銘柄ごとの最後の実開示日）も持つ。
起点が無い古いファイルを読んだときは「最も早い候補」に落ちて、件数を warning に出す。
